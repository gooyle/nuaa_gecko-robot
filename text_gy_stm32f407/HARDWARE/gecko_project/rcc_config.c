...
*input:none
*output:none
*remarks：focus on 25M external XTAL sysclock setted as 168M
********************************************************************************************************/ 
...
{		
	...
	if( HSEStartUpStatus== SUCCESS )     //等待外部时钟启动
	{
		...
		RCC_PCLK1Config(RCC_HCLK_Div4);    //PCLK（APB1）时钟为HCLK时钟4分频
		RCC_PCLK2Config(RCC_HCLK_Div2);    //PCLK（APB2）时钟为HCLK时钟2分频
		RCC_PLLConfig(RCC_PLLSource_HSE,25,336,2,4);    //PLL时钟配置，外部晶振为25MHz，系统配置为168MHz
